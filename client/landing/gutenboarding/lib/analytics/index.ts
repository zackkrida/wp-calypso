 ...
 */
export function recordOnboardingStart( ref = '', site_count: number, locale: string ): void {
	if ( ! ref ) {
		ref = new URLSearchParams( window.location.search ).get( 'ref' ) || ref;
	}

	trackEventWithFlow( 'calypso_newsite_start', { ref, site_count, user_lang: locale } );
	// Also fire the signup start|complete events. See: pbmFJ6-95-p2
	trackEventWithFlow( 'calypso_signup_start', { ref, site_count, user_lang: locale } );
}
